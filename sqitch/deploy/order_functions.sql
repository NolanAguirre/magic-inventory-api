...
CREATE FUNCTION magic_inventory.place_order(agrs_card_ids UUID[],args_store_id UUID, args_use_id UUID) RETURNS VOID AS $$ -- cards, store id, user who ordered
  DECLARE
    ...
    card_condition magic_inventory.condition_type;
    temp UUID;
  BEGIN
    temp := uuid_generate_v4();
    FOREACH inventory_card_id IN SELECT ARRAY $1
    LOOP
      card_condition := SELECT condition FROM magic_inventory.inventory WHERE id = inventory_card_id;
      INSERT INTO magic_inventory.orders (order_number, card_id, store_id, user_id, order_status, condition) VALUE (temp, inventory_card_id, $2, $3, "sent", card_condition);
