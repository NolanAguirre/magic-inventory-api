...
CREATE FUNCTION magic_inventory.create_magic_card(json) RETURNS magic_inventory.magic_card_type AS $$
  ...
  BEGIN
    ...
    card_statis := (SELECT collectors_number FROM magic_inventory.inventory WHERE (card).name = $1.name AND (card).card_set = $1.card_set);
...
CREATE FUNCTION magic_inventory.update_card_status(magic_inventory.magic_card_type, magic_inventory.card_status_type) RETURNS VOID AS $$
  UPDATE magic_inventory.cards SET (card).
$$ LANGUAGE SQL;
